...
# # ---------- REPORT ROUTES ---------- #
# @app.route('/redflags', methods=['POST', 'GET', 'OPTIONS'])
# @jwt_required()
# def redflags():
...
# @app.route('/interventions', methods=['POST', 'GET', 'OPTIONS'])
# @jwt_required()
# def interventions():
#     if request.method == 'OPTIONS':
#         return '', 200  # Handle preflight request
...
#         intervention = Intervention(title=title, description=description, location=location, user_id=current_user['id'])
...
#         return jsonify({"message": "Password reset link sent if email is registered"}), 200
#     except Exception as e:
#         return jsonify({"error": "Internal Server Error", "details": str(e)}), 500
